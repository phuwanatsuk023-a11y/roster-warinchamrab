...
interface DutyPointsManagementProps {
  ...
  onUpdateDutyPoints?: (dutyPoints: DutyPoint[]) => Promise<void>;
...
export const DutyPointsManagement: React.FC<DutyPointsManagementProps> = ({
  ...
  const malePoints = safeDutyPoints.filter(p => p.gender === 'M');
  const femalePoints = safeDutyPoints.filter(p => p.gender === 'F');
  ...
  const handleAddMale = async (e: React.FormEvent) => {
    ...
    } else if (onUpdateDutyPoints) {
      const newId = safeDutyPoints.length > 0 ? Math.max(...safeDutyPoints.map(p => p.id)) + 1 : 1;
      const newPt: DutyPoint = { id: newId, name: maleInput.trim(), gender: 'M' };
      await onUpdateDutyPoints([...safeDutyPoints, newPt]);
    }
    setMaleInput('');
  };
  ...
  const handleAddFemale = async (e: React.FormEvent) => {
    ...
    if (onAddDutyPoint) {
      ...
      const newId = safeDutyPoints.length > 0 ? Math.max(...safeDutyPoints.map(p => p.id)) + 1 : 1;
      const newPt: DutyPoint = { id: newId, name: femaleInput.trim(), gender: 'F' };
      await onUpdateDutyPoints([...safeDutyPoints, newPt]);
    }
    setFemaleInput('');
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (onDeleteDutyPoint) {
      await onDeleteDutyPoint(deletingPoint.id);
    } else if (onUpdateDutyPoints) {
      await onUpdateDutyPoints(safeDutyPoints.filter(p => p.id !== deletingPoint.id));
    }

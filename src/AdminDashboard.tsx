...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const activePersonnel = safePersonnel.filter(p => p.status === 'active');
  ...
  const malePoints = safeDutyPoints.filter(p => p.gender === 'M').length;
  const femalePoints = safeDutyPoints.filter(p => p.gender === 'F').length;

  const currentMonth = new Date().getMonth();
  const currentYear = new Date().getFullYear();
  const monthHolidays = safeHolidays.filter(h => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-sky-900 via-sky-800 to-blue-900 text-white rounded-2xl p-6 border border-sky-700/50 shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative overflow-hidden">
        <div className="relative z-10">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-700/60 text-sky-200 text-xs font-semibold mb-2 border border-sky-500/40">
        ...
        <div className="relative z-10 flex flex-wrap gap-2.5">
          ...
          <button
            ...
            className="px-3.5 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white text-xs font-bold shadow-md border border-sky-400/40 flex items-center gap-1.5 transition-all"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-white text-sky-950 hover:bg-sky-50 text-xs font-black shadow-md flex items-center gap-1.5 transition-all"
          >
            <FileText className="w-4 h-4 text-sky-900" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div
          ...
          className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm hover:border-sky-500 hover:shadow-md transition-all cursor-pointer group"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500">บุคลากรทั้งหมด</span>
            <div className="w-8 h-8 rounded-lg bg-sky-50 dark:bg-sky-950/50 text-sky-700 dark:text-sky-300 flex items-center justify-center group-hover:scale-110 transition-transform">
          ...
          <div className="flex items-center gap-2 mt-2 text-xs font-medium">
            <span className="text-sky-700 dark:text-sky-400 font-bold">ชาย {males.length}</span>
            <span className="text-slate-300">•</span>
            <span className="text-blue-600 dark:text-blue-400 font-bold">หญิง {females.length}</span>
          </div>
        </div>
        ...
        >
          ...
          <p className="text-2xl font-extrabold text-sky-700 dark:text-sky-400 mt-2">
            {dutyGuards.length}{' '}
            <span className="text-xs font-normal text-slate-500">คน</span>
          </p>
          <div className="flex items-center gap-2 mt-2 text-xs font-medium">
            <span className="text-sky-700 dark:text-sky-400 font-bold">ชาย {maleDuty}</span>
            <span className="text-slate-300">•</span>
            <span className="text-blue-600 dark:text-blue-400 font-bold">หญิง {femaleDuty}</span>
          </div>
        </div>

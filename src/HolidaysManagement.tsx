...
export const HolidaysManagement: React.FC<HolidaysManagementProps> = ({
  ...
  }, [safeHolidays, searchTerm, selectedYear, selectedType]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (onUpdateHolidays) {
        const newId = safeHolidays.length > 0 ? Math.max(...safeHolidays.map(h => h.id)) + 1 : 1;
        const newH: Holiday = { id: newId, holiday_date: dateVal, name: nameVal.trim(), type: typeVal };
        await onUpdateHolidays([...safeHolidays, newH]);
      }
      setDateVal('');
      setNameVal('');
      setIsAddOpen(false);

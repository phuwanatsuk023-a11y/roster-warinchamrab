...
interface PersonnelManagementProps {
  ...
  onUpdatePersonnel?: (personOrList: any) => void | Promise<void>;
...
export const PersonnelManagement: React.FC<PersonnelManagementProps> = ({
  ...
  }, [safePersonnel, searchQuery, filterGender, filterRole, filterPoint]);
  ...
  const handleDrop = async (e: React.DragEvent, targetId: number) => {
    ...
    const sourceIdx = safePersonnel.findIndex(p => p.db_id === draggedId);
    const targetIdx = safePersonnel.findIndex(p => p.db_id === targetId);
    if (sourceIdx === -1 || targetIdx === -1) return;

    const updated = [...safePersonnel];
    const [moved] = updated.splice(sourceIdx, 1);
    updated.splice(targetIdx, 0, moved);

    // Update orderIndex
    const reordered = updated.map((p, idx) => ({ ...p, orderIndex: idx + 1 }));
    if (onReorderPersonnel) {
      await onReorderPersonnel(reordered);
    } else if (onUpdatePersonnel) {
      onUpdatePersonnel(reordered);
    }
    setDraggedId(null);
  };
  ...
  const handleEmpSearch = async (val: string) => {
    ...
    try {
      if (onSearchEmployeeDirectory) {
        const results = await onSearchEmployeeDirectory(val);
        setEmpSearchResults(results);
      } else {
        const results = await import('./apiService').then(m => m.apiService.searchEmployee(val));
        setEmpSearchResults(results);
      }
    } catch {
      setEmpSearchResults([]);
  ...
  const selectEmployee = (emp: EmployeeDirectoryItem) => {
    ...
    setAddForm(prev => ({
      ...
      dept: emp.dept,
      dutyPoint: g === 'M' ? (safeDutyPoints.find(dp => dp.gender === 'M')?.name || '') : '',
    }));
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    const newPersonData: Omit<Personnel, 'db_id'> = {
      id: addForm.id || `EMP${String(safePersonnel.length + 1).padStart(3, '0')}`,
      ...
      orderIndex: safePersonnel.length + 1,
    };

    if (onAddPersonnel) {
      await onAddPersonnel(newPersonData);
    } else {
      const newDbId = safePersonnel.length > 0 ? Math.max(...safePersonnel.map(p => p.db_id)) + 1 : 1;
      const newPerson: Personnel = { ...newPersonData, db_id: newDbId };
      const updatedList = [...safePersonnel, newPerson];
      if (onUpdatePersonnel) {
        onUpdatePersonnel(updatedList);
      }
    }
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (onUpdatePersonnel) {
      // If parent expects list or item
      const idx = safePersonnel.findIndex(p => p.db_id === editingPerson.db_id);
      if (idx !== -1) {
        const updatedList = [...safePersonnel];
        updatedList[idx] = editingPerson;
        onUpdatePersonnel(updatedList);
      } else {
        onUpdatePersonnel(editingPerson);
      }
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (onDeletePersonnel) {
      ...
      const filtered = safePersonnel.filter(p => p.db_id !== deletingPerson.db_id);
      onUpdatePersonnel(filtered);
    }

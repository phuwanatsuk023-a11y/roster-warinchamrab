...
class ApiService {
  ...
  public async testConnection(url?: string): Promise<boolean> {
    const targetUrl = url || this.getSettings().appsScriptUrl;
    if (!targetUrl) return false;
    try {
      const urlObj = new URL(targetUrl);
      urlObj.searchParams.set('action', 'test');
      const res = await fetch(urlObj.toString(), {
        method: 'GET',
  ...
  // PERSONNEL API
  public getLocalPersonnel(): Personnel[] {
  ...
  // DUTY POINTS API
  public getLocalDutyPoints(): DutyPoint[] {
  ...
  public async getDutyPoints(): Promise<DutyPoint[]> {
    ...
    return this.getLocalDutyPoints();
  }
  ...
  public async saveDutyPoints(points: DutyPoint[]): Promise<{ count: number; message: string }> {
    saveToStorage(LS_KEY_POINTS, points);
    const settings = this.getSettings();
    if (settings.appsScriptUrl) {
      try {
        const res = await this.callAppsScript('sync_all_duty_points', 'POST', { duty_points: points });
        return {
          count: points.length,
          message: res?.message || `ซิงก์ข้อมูลจุดประจำเวร ${points.length} จุดขึ้น Google Sheet สำเร็จแล้ว`,
        };
      } catch (err: any) {
        console.warn('Apps Script sync_all_duty_points error:', err);
        throw new Error(`ซิงก์จุดประจำเวรขึ้น Google Sheet ไม่สำเร็จ: ${err.message || err}`);
      }
    }
    return {
      count: points.length,
      message: `บันทึกข้อมูลจุดประจำเวร ${points.length} จุดลงในเครื่องเรียบร้อย`,
    };
  }

  // HOLIDAYS API
  public getLocalHolidays(): Holiday[] {
  ...
  // USER MANAGEMENT API
  public getLocalUsers(): UserAccount[] {
  ...
  public getAllLocalSavedRosters(): Array<{
    year: number;
    month: number;
    gender: 'M' | 'F' | 'ALL';
    items: SavedRosterScheduleRow[];
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith(LS_KEY_ROSTER_PREFIX) && !key.endsWith('_time')) {
          const parts = key.replace(LS_KEY_ROSTER_PREFIX, '').split('_');
          if (parts.length >= 3) {
            const year = parseInt(parts[0], 10);
            const month = parseInt(parts[1], 10);
            const gender = parts[2] as 'M' | 'F' | 'ALL';
            const items = loadFromStorage<SavedRosterScheduleRow[]>(key, []);
            const savedTime = loadFromStorage<string | null>(`${key}_time`, null);
            if (items && items.length > 0) {
              results.push({ year, month, gender, items, savedTime });
            }
          }
        }
      }
    } catch (e) {
      console.warn('Error reading saved rosters from localStorage:', e);
    }

    return results;
  }

  public async syncAllSavedRostersToSheet(): Promise<{ count: number; totalRows: number; message: string }> {
    const savedList = this.getAllLocalSavedRosters();
    if (savedList.length === 0) {
      return {
        count: 0,
        totalRows: 0,
        message: 'ไม่พบข้อมูลตารางเวรที่บันทึกไว้ในเครื่อง กรุณาสร้างและบันทึกตารางเวรในเมนู "สร้างตารางเวร" ก่อน',
      };
    }
    ...
    for (const entry of savedList) {
      await this.callAppsScript('save_roster_schedule', 'POST', {
        month: entry.month,
        year: entry.year,
        gender: entry.gender,
        items: entry.items,
      });
      totalRows += entry.items.length;
    }

    return {
      count: savedList.length,
      totalRows,
      message: `ซิงก์ข้อมูลตารางเวรที่บันทึกไว้ทั้งหมด ${savedList.length} ชุด (${totalRows} รายการ) ขึ้น Google Sheet สำเร็จแล้ว`,
    };
  }

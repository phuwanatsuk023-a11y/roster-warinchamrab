...
interface GoogleSheetSettingsModalProps {
  ...
  onSyncRostersPush?: () => Promise<void>;
...
export const GoogleSheetSettingsModal: React.FC<GoogleSheetSettingsModalProps> = ({
  ...
  const handlePush = async () => {
    setIsSyncing(true);
    setSyncStatus('กำลังส่งข้อมูลบุคลากร ผู้ใช้ และวันหยุดขึ้น Google Sheet...');
    try {
      await onSyncPushToSheet();
      setSyncStatus('ส่งข้อมูลหลักขึ้น Google Sheet สำเร็จเรียบร้อย!');
    } catch (e: any) {
  ...
  const handlePushRosters = async () => {
    if (!onSyncRostersPush) return;
    setIsSyncing(true);
    setSyncStatus('กำลังส่งข้อมูลตารางเวรที่บันทึกไว้ขึ้น Google Sheet API...');
    try {
      await onSyncRostersPush();
      setSyncStatus('ส่งข้อมูลตารางเวรทั้งหมดขึ้น Google Sheet สำเร็จเรียบร้อย!');
    } catch (e: any) {
      setSyncStatus(`เกิดข้อผิดพลาด: ${e.message}`);
    } finally {
      setIsSyncing(false);
                ...
                <div className="bg-slate-50 dark:bg-slate-700/50 p-3.5 rounded-xl border border-slate-200 dark:border-slate-600 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={handlePush}
                      disabled={isSyncing}
                      className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold flex items-center gap-1 shadow-sm disabled:opacity-50"
                    >
                      <Upload className="w-3.5 h-3.5" />
                      <span>ส่งข้อมูลบุคลากร/ผู้ใช้/วันหยุด ไป Sheet</span>
                    </button>
                    {onSyncRostersPush && (
                      <button
                        onClick={handlePushRosters}
                        disabled={isSyncing}
                        className="px-3 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold flex items-center gap-1 shadow-sm disabled:opacity-50"
                      >
                        <FileSpreadsheet className="w-3.5 h-3.5" />
                        <span>ส่งข้อมูลตารางเวรที่บันทึกไว้ ({savedRosterCount} ชุด) ไป Sheet API</span>
                      </button>
                    )}
                    <button
                      onClick={handlePull}
                      disabled={isSyncing}
                      className="px-3 py-1.5 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold flex items-center gap-1 shadow-sm disabled:opacity-50"
                    >
